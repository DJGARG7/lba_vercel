...
const fetchAllPublicData = (req, res) => {
    ...
    if (
        !(
            typeof pub_table === "string" &&
            typeof pub_criteria === "string" &&
            typeof data.pub_criteria_value === "string"
        )
    ) {
        res.status(400).send("Invalid Request Body");
        return;
    ...
    if (pub_table === "pub_lba_category" && pub_criteria === "all") {
        ...
        pub_table === "pub_lba_image" &&
        pub_criteria === "carousel_flag_Y"
    ) {
        query = "SELECT * from lba_image where carousel_flag = ?;";
        queryParam = ["Y"];
    } else if (pub_table === "pub_lba_product" && pub_criteria === "keyword") {
        query =
            "SELECT * from lba_product where product_deleted_flag = 'N' and lower(product_name) like ?;";
        queryParam = [`%${data.pub_criteria_value.toLowerCase()}%`];
    } else if (
        pub_table === "pub_lba_product" &&
        pub_criteria === "category_id"
    ) {
        query =
            "SELECT * from lba_product where product_deleted_flag = 'N' and category_id = ?;";
        queryParam = [data.pub_criteria_value];
    } else {
        query =
            "SELECT * from " +
            pub_table.substring(4) +
            " where " +
            pub_criteria +
            " = ?;";
        queryParam = [data.pub_criteria_value];
    ...
    try {
        db.query(query, queryParam, (error, results) => {
